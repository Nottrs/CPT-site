...
function MagButton({ hash, children }) {
...
function MobileAnchorLink({ hash, onClose, children }) {
  const goTo = useAnchorNav()
  return (
    <button
      onClick={() => { goTo(hash); onClose() }}
      style={{ display: 'block', width: '100%', textAlign: 'left', padding: '14px 0', fontSize: 18, fontWeight: 700, borderBottom: '1px solid rgba(255,255,255,0.05)', fontFamily: 'Syne, sans-serif', color: '#e8e8f0', background: 'none', border: 'none', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}
    >
      {children}
    </button>
  )
...
export default function Navbar() {
  ...
  const goToContact = useAnchorNav()
    ...
    >
      <div style={{
        ...
        <nav style={{ display: 'flex', gap: 38, alignItems: 'center' }}
          ...
          <MagButton hash="contact">Get Started</MagButton>
        </nav>
            ...
            <div style={{ padding: '12px 28px 24px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              {links.map((l, i) => (
                <motion.div key={l.label} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                  {l.anchor
                    ? <MobileAnchorLink hash={l.href.split('#')[1]} onClose={() => setOpen(false)}>{l.label}</MobileAnchorLink>
              ...
              <button
                onClick={() => { goToContact('contact'); setOpen(false) }}
                style={{
                  marginTop: 16, display: 'block', width: '100%', padding: '14px 0', textAlign: 'center',
